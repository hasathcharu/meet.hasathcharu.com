...
export default function UserDashBoard(props){
    ...
    return(
        <div className={styles.dashboard}>
            ...
            <LayoutGroup>
            {meetings?.length===0 && !liveMeeting?.topic && <h1>You have no assigned meetings yet.</h1>}
            {meetings?.length!==0 && 
                <motion.div className={styles.meetings} layout>
                        {meetings.length>0 && <motion.h1 layout>{liveMeeting?.topic?'Other ':'Your '}Meetings</motion.h1>}
