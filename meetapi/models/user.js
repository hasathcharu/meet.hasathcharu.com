...
module.exports = class User {
  ...
  async save(update = 0) {
    try {
      ...
      const result = await db.execute(emailQuery, emailQueryArray);
      if (result[0]?.length != 0) throw new Error('Email Error');
      let saved;
      if (update) {
      ...
      if (saved[0]?.affectedRows == 1) return 'Success';
      if (saved[0]?.affectedRows == 0 && update)
        throw new Error('User Not Found');
      throw new Error();
    } catch (err) {
      if (err.message == 'Email Error') return 'Email Error';
      if (err.message == 'User Not Found') return 'User Not Found';
      return 'Fail';
    }
  }
  async assignLink(link_id) {
    try {
      ...
      if (err?.code == 'ER_NO_REFERENCED_ROW_2') return 'Not Found';
  ...
  async authenticate() {
    try {
      ...
      if (result[0]?.length == 0) return 'Failed Auth';
      throw new Error();
    } catch (error) {
      return 'Fail';
    }
  ...
  async getUnassignedLinks(search = null) {
    ...
    try {
      const result = await db.execute(query, queryArray);
      if (result[0]) return result[0];
      throw new Error();
    } catch {
      return 'Fail';
  ...
  async getIfAnyOtherLive() {
    try {
      ...
      if (result[0][0].S !== undefined) return result[0][0].S;
      throw new Error();
    } catch {
      return 'Fail';
    }
  ...
  async checkAssigned(link_id) {
    try {
      ...
      if (result[0][0].C !== undefined) return result[0][0].C;
      throw new Error();
    } catch {
      return 'Fail';
    }
